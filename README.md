...
| 数据模型 | 数组、`LazyRowSource`、`FlatRowModel`、`TreeRowModel`、数组列与惰性列 |
| 条件格式 | 文本、背景、图标、二/三色阶、正负数据条、优先级与 `stopIfTrue` |
| 输出与集成 | Excel、CSV、当前视口图片、命令式 API、数据坐标回调、`localeText`、ARIA grid/treegrid |
...
Studio 用于展示两个 npm 包的组合能力，而不是生产运行时依赖。

| Tab | 内容 |
| --- | --- |
| 能力总览 | 三层职责、公开包和能力边界 |
| 组件展厅 | 以多个独立、可交互的小表格展示固定、尺寸、选择、复制、合并、自定义渲染等能力；每项可查看源码 |
| 经营分析 | BI 复合维度与指标；树形和同列纵向相邻同值合并可独立开启，同时启用时按兄弟节点边界断开合并 |
