...
Studio 用于展示两个 npm 包的组合能力，而不是生产运行时依赖。当前有 4 个顶层 Tab：

| Tab | 内容 |
| --- | --- |
| 介绍 | 独立呈现 Studio、应用表格与渲染表格三层架构、发布边界和能力摘要，不占用表格演示区域 |
| 组件展厅 | 按基础/进阶分组提供 11 个可交互示例；每项直接编辑实现该 Demo 的 TSX 文件并实时刷新预览，覆盖惰性行列、多级树、命令式 API 与 Excel/CSV/PNG 导出 |
