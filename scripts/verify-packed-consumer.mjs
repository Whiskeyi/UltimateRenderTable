...
try {
  ...
  await writeFile(
    ...
    [
      ...
      "if (JSON.stringify(core.parseTSV('North\\t42')) !== JSON.stringify([['North', '42']])) throw new Error('Core parseTSV export failed')",
      "if (core.rangeToTSV({ rowStart: 0, rowEnd: 0, columnStart: 0, columnEnd: 1 }, ({ column }) => column) !== '0\\t1') throw new Error('Core rangeToTSV export failed')",
      "if (!core.rangesIntersect({ rowStart: 0, rowEnd: 1, columnStart: 0, columnEnd: 1 }, { rowStart: 1, rowEnd: 2, columnStart: 1, columnEnd: 2 })) throw new Error('Core rangesIntersect export failed')",
      "if (core.moveTabAddress({ row: 0, column: 0 }, { rowStart: 0, rowEnd: 1, columnStart: 0, columnEnd: 1 }, false, () => undefined).column !== 1) throw new Error('Core moveTabAddress export failed')",
      'const ssrWarnings = []',
