...
选区常驻状态是一个闭区间矩形和独立 anchor/focus，空间为 `O(1)`。点击、拖拽、Shift、方向键、Tab 与 Enter 共用坐标导航；进入或离开合并区域时按整个矩形移动。Tab 在选区遍历越界时不再被 Core 拦截，而是释放浏览器原生焦点顺序；树按钮、列宽 separator 等内部可操作控件仍可能成为下一个停靠点，并不承诺一次按键退出整个组件。

`mobileInteraction` 默认按粗指针与触控能力自动启用，也可强制开启或关闭。默认 `scrollAxisLock: 'dominant'`：纵向手势由浏览器原生滚动，明确的横向手势越过 `tapSlop` 后只更新 `scrollLeft` 并保持单轴惯性；需要浏览器接管双轴平移时可设为 `'native'`。只有未越过 `tapSlop` 且未发生滚动的 pointer 才提交点击选择。活动 cell 暴露选区手柄，拖动后沿同一坐标协议扩选；接近边缘时使用独立阈值继续滚动。选区存在时可显示带安全区间距的复制动作，并通过可覆盖 labels 提供本地化反馈。
