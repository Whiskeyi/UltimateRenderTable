...
Studio 有 4 个顶层 Tab：介绍、组件展厅、经营分析和条件格式。介绍页在单屏内交互呈现三层架构、发布边界与能力摘要；组件展厅按基础/进阶分组挂载 11 个可交互示例，每项从实际 TSX 文件加载源码并在浏览器中实时编译预览，不在表格上下叠加说明栏；经营分析负责组合业务维度、树形和同列合并。默认日常档为 `1K × 40`、行列 overscan `2 / 1`、自动行高关闭，`100K × 100K` 仅作为手动压力预设。
