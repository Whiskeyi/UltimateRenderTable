...
| 能力 | 状态 | 边界 |
| --- | --- | --- |
| 数组与惰性数据源 | Available | `rows`、`rowSource`、`rowModel` 在类型层互斥 |
| 数组与惰性列 | Available | `columns` 或 `columnCount + getColumn` 二选一 |
| 平铺与树形 | Available | `TreeRowModel` 支持同步/异步子节点与真实可见序列 |
...
| Excel / CSV | Available | 客户端物化，默认 1,000,000-cell 上限；Excel 受工作表规格限制 |
| 图片导出 | Partial | 捕获当前虚拟化视口，不生成完整逻辑长图 |
...
| 能力 | 状态 | 边界 |
| --- | --- | --- |
| 能力总览 | Available | 说明三层职责和公开边界 |
| 组件展厅 | Available | 多个独立可交互表格组件，每项支持查看源码 |
| 经营分析 | Available | 树形与同列纵向相邻同值合并可独立开启；同时启用时默认按兄弟节点边界断开合并 |
