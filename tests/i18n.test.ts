...
describe('translate', () => {
  it('returns Chinese and English messages from the selected locale', () => {
    ...
    expect(translate('zh-CN', 'error.scenario')).toContain('intro / gallery / analysis / spreadsheet')
    expect(translate('en-US', 'error.scenario')).toContain('intro, gallery, analysis, or spreadsheet')
    expect(translate('zh-CN', 'scenario.gallery')).toBe('组件展厅')
    expect(translate('en-US', 'scenario.gallery')).toBe('Component gallery')
    expect(translate('zh-CN', 'scenario.spreadsheet')).toBe('电子表格')
    expect(translate('en-US', 'scenario.spreadsheet')).toBe('Spreadsheet')
    ...
    expect(translate('en-US', 'gallery.group.production')).toBe('Production cases')
    expect(translate('zh-CN', 'gallery.orders.title')).toBe('订单履约中心')
    expect(translate('en-US', 'gallery.budget.title')).toBe('Annual budget matrix')
