...
describe('translate', () => {
  it('returns Chinese and English messages from the selected locale', () => {
    ...
    expect(translate('zh-CN', 'gallery.capabilities.basic')).toContain('拖选、越界滚动与 Shift 扩选')
    expect(translate('zh-CN', 'gallery.capabilities.basic')).toContain('方向键/Tab/Enter')
    expect(translate('en-US', 'gallery.capabilities.advanced')).toContain('arbitrary-depth async trees')
    expect(translate('en-US', 'gallery.capabilities.advanced')).toContain('rule priority')
